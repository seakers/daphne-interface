class Tutorial {
    ...
    start_intro(objects, messages, classname, callback){
        ...
        if(callback){
            ...
            this.intro.setOptions({steps:steps,tooltipClass:classname}).start(); 
        }
    }    
    ...
    run_tutorial(){
        ...
        sequence.push({"highlight":d3.select('#arch_info_display_table')._groups[0][0],
        ...
        sequence.push({"highlight":d3.select('#arch_info_display_table')._groups[0][0],
                            "text":"The real names of orbits and instruments were changed to numbers and alphabetical letters repectively to make the task simpler. There are total 5 candidate orbits, and 12 available 12 measurement instruments."
                        });           // 6 instead of 12?
        ...
        sequence.push({"highlight":d3.select('#main_plot_block')._groups[0][0],
        ...
        sequence.push({"highlight":d3.select('.columns > div > section')._groups[0][0],
        ...
        sequence.push({"highlight":d3.select('.navbar-menu')._groups[0][0],
