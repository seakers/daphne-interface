    ...
    <div style="max-height: 600px; overflow-y: auto; overflow-x: hidden">
        ...
        <div class="box">
            <table class='table is-fullwidth'>
                ...
                <tbody>
                <tr v-for="(row, idx) in instrument_list" :key="idx">
                    <td>{{ idx }}</td>
                    <td>{{ row.name }}</td>
                    <td v-if="row.is_assigned"><button class="button is-danger is-small" v-on:click.prevent="remove_instrument(idx, row)">Remove</button></td>
                    <td v-if="!row.is_assigned"><button class="button is-success is-small" v-on:click.prevent="assign_instrument(idx, row)">Add</button></td>
                </tr>
                </tbody>
            </table>
        </div>
