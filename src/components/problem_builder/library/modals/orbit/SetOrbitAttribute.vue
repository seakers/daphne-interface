        ...
        <div class="modal-content" style="width: 400px !important;">
            <article class="message">
                ...
                <div class="modal-clone-body" v-if="local_attribute.has_accepted">
                    ...
                    <!-- GLOBAL ATTRIBUTE -->
                    <select v-model="new_name" v-if="!update">
                        <option v-for="option in local_attribute.Join__Orbit_Attribute_Values" v-bind:value="option.Accepted_Value.value" :key="option.Accepted_Value.value">
                            {{ option.Accepted_Value.value }}
                        </option>
                    </select>
                </div>
...
<script>
    import { SetOrbitAttributeValue, InsertOrbitAttributeValue } from '../../../../../scripts/orbit-queries';
    import { mapState, mapGetters } from 'vuex';

    export default {
        name: 'set-orbit-attribute',
        props: ['isActive', 'local_attribute', 'update', 'orbit_id'],
        data() {
            return {
                new_name: '',
                Orbit: [],
            ...
            async set_value(){
                ...
                if(!this.update){ // update == false
                    let insert_orb_attribute = await this.$apollo.mutate({
                        mutation: InsertOrbitAttributeValue,

                        variables: {
                            orbit_id: this.orbit_id,
                            group_id: this.selected_group_id,
                            orbit_attribute_id: this.local_attribute.id,
                            value: this.new_name,
                        },

                        update: (cache, { data: { insert_orbit_attr } }) => {
                            // Read the data from our cache for this query.
                            // eslint-disable-next-line
                            console.log(insert_orbit_attr);
                        },
                    });
                    ...
                    let update_orb_attribute = await this.$apollo.mutate({
                        mutation: SetOrbitAttributeValue,

                        variables: {
                            orbit_attribute_id: this.local_attribute.join_id,
                            value: this.new_name,
                        },

                        update: (cache, { data: { set_orbit_attr } }) => {
                            // Read the data from our cache for this query.
                            // eslint-disable-next-line
                            console.log(set_orbit_attr);
                        },
                    });
