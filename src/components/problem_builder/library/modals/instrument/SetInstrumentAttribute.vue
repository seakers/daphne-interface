        ...
        <div class="modal-content" style="width: 400px !important;">
            <article class="message">
                ...
                <div class="modal-clone-body" v-if="local_attribute.has_accepted">
                    ...
                    <!-- GLOBAL ATTRIBUTE -->
                    <select v-model="new_name" v-if="!update">
                        <option v-for="option in local_attribute.Join__Instrument_Attribute_Values" v-bind:value="option.Accepted_Value.value" :key="option.Accepted_Value.value">
                            {{ option.Accepted_Value.value }}
                        </option>
                    </select>
                </div>
...
<script>
    import { SetInstrumentAttributeValue, InsertInstrumentAttributeValue } from '../../../../../scripts/instrument-queries';
    import { mapState, mapGetters } from 'vuex';

    export default {
        name: 'set-instrument-attribute',
        props: ['isActive', 'local_attribute', 'update', 'instrument_id', 'problem_id'],
        data() {
            return {
                new_name: '',
                Instrument: [],
            ...
            async set_value(){
                ...
                if(!this.update){ // update == false
                    let insert_orb_attribute = await this.$apollo.mutate({
                        mutation: InsertInstrumentAttributeValue,

                        variables: {
                            instrument_id: this.instrument_id,
                            group_id: this.selected_group_id,
                            instrument_attribute_id: this.local_attribute.id,
                            problem_id: this.problem_id,
                            value: this.new_name,
                        },

                        update: (cache, { data: { insert_instrument_attr } }) => {
                            // Read the data from our cache for this query.
                            // eslint-disable-next-line
                            console.log(insert_instrument_attr);
                        },
                    });
                    ...
                    let update_orb_attribute = await this.$apollo.mutate({
                        mutation: SetInstrumentAttributeValue,

                        variables: {
                            instrument_attribute_id: this.local_attribute.join_id,
                            value: this.new_name,
                        },

                        update: (cache, { data: { set_instrument_attr } }) => {
                            // Read the data from our cache for this query.
                            // eslint-disable-next-line
                            console.log(set_instrument_attr);
                        },
                    });
