        ...
        <div class="modal-content" style="width: 400px !important;">
            <article class="message">
                ...
                <div class="modal-clone-body" v-if="local_attribute.has_accepted">
                    ...
                    <!-- GLOBAL ATTRIBUTE -->
                    <select v-model="new_name" v-if="!update">
                        <option v-for="option in local_attribute.Join__Launch_Vehicle_Attribute_Values" v-bind:value="option.Accepted_Value.value" :key="option.Accepted_Value.value">
                            {{ option.Accepted_Value.value }}
                        </option>
                    </select>
                </div>
...
<script>
    import { SetLaunchVehicleAttributeValue, InsertLaunchVehicleAttributeValue } from '../../../../../scripts/launch-vehicle-queries';
    import { mapState, mapGetters } from 'vuex';

    export default {
        name: 'set-launch-vehicle-attribute',
        props: ['isActive', 'local_attribute', 'update', 'launch_vehicle_id'],
        data() {
            return {
                new_name: '',
                Launch_Vehicle: [],
            ...
            async set_value(){
                ...
                if(!this.update){ // update == false
                    let insert_orb_attribute = await this.$apollo.mutate({
                        mutation: InsertLaunchVehicleAttributeValue,

                        variables: {
                            launch_vehicle_id: this.launch_vehicle_id,
                            group_id: this.selected_group_id,
                            launch_vehicle_attribute_id: this.local_attribute.id,
                            value: this.new_name,
                        },

                        update: (cache, { data: { insert_launch_vehicle_attr } }) => {
                            // Read the data from our cache for this query.
                            // eslint-disable-next-line
                            console.log(insert_launch_vehicle_attr);
                        },
                    });
                    ...
                    let update_orb_attribute = await this.$apollo.mutate({
                        mutation: SetLaunchVehicleAttributeValue,

                        variables: {
                            launch_vehicle_attribute_id: this.local_attribute.join_id,
                            value: this.new_name,
                        },

                        update: (cache, { data: { set_launch_vehicle_attr } }) => {
                            // Read the data from our cache for this query.
                            // eslint-disable-next-line
                            console.log(set_launch_vehicle_attr);
                        },
                    });
